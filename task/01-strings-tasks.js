...
function getRectangleString(width, height) {
  let line = '─';
  let lineSpace = `│${' '.repeat(width - 2)}│\n`;
  return `┌${line.repeat(width - 2)}┐\n`+ lineSpace.repeat(height - 2) +
   `└${line.repeat(width - 2)}┘\n`;
}
...
function encodeToRot13(str) {
  let alphabetLatin = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  let alphabetRot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/\w/g, (letter, i, str) => {
    return alphabetRot13[alphabetLatin.indexOf(letter)];
  });
}
